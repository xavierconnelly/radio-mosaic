<script>
    ...
    let activeSlug = $derived(stationData[activeIndex]?.slug);
    let hoveredSlug = $derived(stationData[ui.hover]?.slug);
    ...
    let occupiedCells = $derived(new Set([
        ...stationData.map(s => `${s.col}-${s.row}`),
        // UTC label row
        ...Array.from({length: 20}, (_, i) => `${i+1}-9`)
    ]));

    let emptyCells = $derived(allCells.filter(c => !occupiedCells.has(`${c.col}-${c.row}`)));
</script>
