...
function Scroller_IMAGES($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    for (let i = 0, $$length = each_array.length; i < $$length; i++) {
      ...
      $$renderer2.push(`<a data-sveltekit-noscroll=""${attr("href", `/${stringify(slug)}`)} class="face svelte-1qzm4qi"${attr_style(`transform: rotateY(${stringify(i * angleStep)}deg) translateZ(${stringify(radius)}em)`)}><img${attr("src", `../images/glimpse/${stringify(item.slug)}.webp`)}${attr("alt", `an aerial landscapr view of ${stringify(item.city)}`)} class="svelte-1qzm4qi"/> ${escape_html(item.name)}</a>`);
    }
...
function ClockBigHand($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let i = $$props["i"];
    let name = $$props["name"];
    let slug2 = $$props["slug"];
    let utc = $$props["utc"];
    let tint = $$props["tint"];
    let clockhand = $$props["clockhand"];
    let radius = 360 / stationData.length;
    $$renderer2.push(`<a${attr("href", `./stations/${stringify(slug2)}`)} class="hand svelte-k5wled"${attr_style(`transform: rotate(calc(${stringify(i)} * ${stringify(radius)}deg)) translateX(50%); color:#${stringify(clockhand)}`)}><span class="details svelte-k5wled"${attr_style(`background-color: #${stringify(tint)}`)}><span class="name svelte-k5wled">${escape_html(name)}</span> <span class="utc svelte-k5wled">${escape_html(utc)}</span></span></a>`);
    bind_props($$props, { i, name, slug: slug2, utc, tint, clockhand });
  });
}
